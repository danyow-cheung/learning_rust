...
fn main() {
    ...
    let lower_right = parse_complex(&args[4])
        .expect("error parsing lowwer  right");
...
// Write the buffer `pixels` ,whose dimension are gived by `bounds` to the file named 'filename'
fn write_image(filename:&str,pixels: &[u8],bounds:(usize,usize))-> Result<(),std::io::Error>{
    let output = File::create(filename)?;
    let encoder = PNGEncoder::new(output);
    encoder.encode(&pixels,bounds.0 as u32,bounds.1 as u32,ColorType::Gray(8))?; // 8位灰度值
    Ok(())
}
